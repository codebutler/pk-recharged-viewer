...
Scriptable harness that runs `Pokemon Recharged Yellow.gba` in mGBA, auto-advances
...
```sh
analysis/tools/run_harness.sh <rom.gba> <label> [max_frames] [period] [input]
# e.g.
analysis/tools/run_harness.sh "Pokemon Recharged Yellow.gba" newgame-spam 20000 600 1
```

- `label` — dumps land in `analysis/dumps/<label>/`
...
The driver copies the ROM to `analysis/dumps/<label>/work/rom.gba` (so mGBA's
`.sav` never lands next to the original ROM), launches mGBA with
`analysis/tools/mgba_dump_harness.lua`, polls for the `DONE` marker, and kills the
...
```sh
HARNESS_SCRIPT=analysis/tools/mgba_inject_harness.lua \
  analysis/tools/run_harness.sh "Pokemon Recharged Yellow.gba" inject 8000
```

## Dump layout

```
analysis/dumps/<label>/
