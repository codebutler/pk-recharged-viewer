...
```sh
tools/run_harness.sh "Pokemon Recharged Yellow.gba" mylabel 20000 600 1
...
--depth 1` commands); without them the generator falls back to throttled HTTP
with a disk cache under `report/.cache/`, then styled placeholders.
...
| `tools/parse_ram.py` | **the product**: dump → game-state JSON | ram-parser agent |
