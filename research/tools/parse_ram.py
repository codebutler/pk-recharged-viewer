...
Offset configuration is layered (later wins):
    1. built-in vanilla pokeemerald offsets (from research/structs.json, verified
       against compiled headers -- see research/structs-notes.md)
    2. offsets-discovered.json   (this tool's own empirical findings, next to script)
    3. research/hack-offsets.json (authoritative hack offsets, if present)
...
Requires only the Python 3 standard library plus research/structs.json (charmap and
substruct permutation). research/gamedata.json, if present, supplies name tables for
...
# Vanilla pokeemerald offsets (research/structs.json). Overlaid by config files.
VANILLA_OFFSETS = {
...
# 4-badge save (research/real-saves/) has 0x860/0x861 SET and 0x87A CLEAR -- so
...
class Config:
    ...
    def _load_hack_offsets(self, data, source):
        """research/hack-offsets.json schema: per-struct sections whose entries carry
...
class GameData:
    """Optional name tables from research/gamedata.json (tolerant of schema)."""
